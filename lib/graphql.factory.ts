...
export class GraphQLFactory {
  ...
  async mergeOptions(
    ...
    const typesResolvers = extend( //[{"Date":"Date"},{"Query":{},"Mutation":{},"Recipe":{},"Subscription":{"recipeAdded":{}}}]
      this.scalarsExplorerService.explore(), //{Date: GraphQLScalarType}
      resolvers,
    );
    ...
    if (options.autoSchemaFile) { // true  
      const autoGeneratedSchema: GraphQLSchema = await this.gqlSchemaBuilder.build(//  从 class 构建 GraphQLSchema
        options.autoSchemaFile,
        options,
        this.resolversExplorerService.getAllCtors(),
      );
      const executableSchema = makeExecutableSchema({ // 合并 resolvers , typeDefs
      ...
      const schemaConfig = this.overrideOrExtendResolvers(
        schema.toConfig(), // schema 覆盖 autoGeneratedSchemaConfig ：resolve，subscribe ，resolverName
        autoGeneratedSchemaConfig,
      );

      schema = new GraphQLSchema(schemaConfig);
      if (options.schemaDirectives) { // false 
      ...
      schema = await transformSchema(schema); // schema => schema
